...
const MapComponent = () => {
    ...
    const fetchBackendSites = async () => {
      try {
        ...
        const response = await fetch(
          "https://hackout2025-backend-d3j9.onrender.com/ml/sites/topp"
        );
        if (!response.ok) {
  ...
  const generateAIRecommendation = async () => {
    ...
    try {
      ...
      const apiUrl = new URL(
        "https://hackout2025-backend-d3j9.onrender.com/ml/sites/city"
      );
  ...
  const refreshData = async () => {
    try {
      ...
      setError(null);
      const response = await fetch(
        "https://hackout2025-backend-d3j9.onrender.com/ml/sites/top"
      );
      if (!response.ok) {
        throw new Error(`Failed to fetch sites data: ${response.status}`);
      }
